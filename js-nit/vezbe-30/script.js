class Pas{
    laj(){
        console.log(`${this.ime} kaze: Av av!`);
    }
    static kolikoImaPas(){
        return Pas.brojPasa;
    }
...
console.log("=== Primer 1:Pas kalsa ===");
...
class PasswordValidator {
  // TVOJ KOD OVDE
  static isValidNumberOfCharacter(password){
    let eightCharacters = password.match(/^(\s*\w){8,}\s*$/);
    if(eightCharacters > 8){
        return;
    }
    else{
        return password;
  ...
  static hasAtLeastOneBigChar(password){
